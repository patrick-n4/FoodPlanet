...
function Signup() {
  ...
  useEffect(()=>{
<<<<<<< HEAD
      document.title = 'create Food planet account';
=======
    ...
    <div className="w-full h-full flex justify-center">
        ...
        <form onSubmit={post} className=" w-1/2 gap-10 flex flex-col items-center justify-center">
            ...
            {inputs.slice(0,2).map((div, index)=>(
                <div key={index} className={`flex ${translate && `translate-x-[-${width ?  24 : 45}rem]`} duration-500 flex-col ${!width ? 'ml-32 min-w-[70%]' : 'min-w-full'} gap-5`}>
                    {index === 0 ? inputs.slice(0,3).map((input, index)=>(
                        <input key={index} className="w-full py-2 rounded-[5px] pl-3 border-2 border-solid outline-none" onChange={getValues} type="text" name={input} placeholder={input}/>
                    )) : 
                    inputs.slice(3).map((input, index) => <div className="relative"><input type={(input === "Password" || input === "Confirm") ? `${index === 1 ? (!password ? 'password' : "text") : (!confirm ? 'password' : "text")}` : "text"} key={index} className="w-full py-2 rounded-[5px] pl-3 border-2 border-solid outline-none" onChange={getValues} name={input} placeholder={input === "Confirm" ? "Confirm you password" : input}/>{(input === "Confirm" || input ==="Password") && <div className="absolute right-3 cursor-pointer top-[20%]" onClick={() => watchP(index)}>{ index === 1 ? (password ? <VisibilityOff />  : <Visibility />) : (confirm ? <VisibilityOff />  : <Visibility />)}</div>}</div>
                    )}
                    <div className="flex justify-end"><Button onClick={() => index === 0 && next()} type={index === 0 ? 'button' : 'submit'} variant="contained" style={{background: "#E08B1F"}}>{index === 0 ? 'Next' : 'Signup'}</Button></div>
                </div>
            ))}
            </div>
            <div className="flex text-xl gap-3 min-w-full justify-center"><span>Already have an account</span><Link to="/login" className="hover:underline text-[blue]">login</Link></div>
        </form>
    </div>
  )
=======
